...
class Rules():
    ...
    def __init__(self):
        self.path = "Travel_com//travel.db"
    ...
    def insert_rules(self):
        ...
        rules = [
            ...
            (4,"culture_museum","'interest=culture' in self.facts or 'interest=local' in self.facts or 'interest=museum' in self.facts","recommend>>best_dic['local_culture']",1.8,"tag","These attractions are recommended for customers who are interested in local culture."),
            ...
            (7,"cabel_montain","'interest=montain' in self.facts or 'interest=montains' in self.facts or 'interest=cabel' in self.facts","recommend>>best_dic['cable']",1.8,"tag","These attractions are recommended for customers who are interested in montains."),
            (8,"china_rec","'interest=china' in self.facts","recommend>>self.ranking(['Beijing','Shanghai','Shenzhen','Guangzhou','Suzhou','Nanjing','Chengdu'],score)",9.0,"contury","These are famous attractions from China."),
            (9,"japan_rec","'interest=japan' in self.facts","recommend>>self.ranking(['Tokyo','Osaka','Kyoto'],score)",9.0,"contury","These are famous destinations from Japan."),
            (10,"malaysia_rec","'interest=malaysia' in self.facts","recommend>>self.ranking(['Kuala Lumpur','Maleka','Penang'],score)",9.0,"contury","These are famous destinations from Malaysia"),
            (11,"us_rec","'interest=us' in self.facts or 'interest=usa' in self.facts or 'interest=united states' in self.facts","recommend>>self.ranking(['New York','San Frencisco','Los Angeles','Las Vegas'],score)",9.0,"contury","These are famous attractions from the US"),
            (12,"brit_rec","'interest=uk' in self.facts or 'interest=britain' in self.facts","recommend>>self.ranking(['London','Manchester'],score)", 9.0,"contury","These are famous attractions from the UK."),
        ]
